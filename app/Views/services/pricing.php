			...
			<?php foreach($packages as $k => $package){ ?>
				<div class="col-md-3 mx-3" data-animate-in="fadeInUp|0.<?=$k+1?>">
					<div class="bdr-2 bdr-default hov-bdr-dark hov-shadow-large anim rd-10 align-c pos-rel">
						...
						<?php } ?>
						<h2 class="title small pd-20 bdr-b-2 bdr-op-1"><?=$package['package_name']?></h2>
						<div class="pd-tiny">
							<p class="lh-1 mr-b-30 txt-dark"><span class="fs50"><?=$package['package_price']?></span><?=is_numeric($package['package_price']) ? 'LKR' : '' ?></p>
							<?php 
								foreach($package['features'] as $feature){
								if($feature['f_type'] == 'main'){
							?>
								<p class="pric-sub title-sub f-2 small"><?=$feature['f_desc']?></p>
							<?php }} ?>
							<hr>
							<a href="/contact?package=<?=$package['package_name']?>" class="btn btn-default small solid">Get started now</a>
						</div>
					</div>
